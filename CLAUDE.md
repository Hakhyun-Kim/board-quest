# 판 위의 원정대 (Board Quest)

길을 골라 나아가고, 큰 판 위에서 **턴제로 겨루는 전술 원정** 게임. 모바일 우선 웹앱 (React + Vite + TypeScript + three.js/r3f).
자매작 **백층 던전**(https://github.com/Hakhyun-Kim/dungeon100)에서 파생 — 절차 생성·외부 에셋 금지·검증 가능성 원칙을 그대로 잇되, **실시간 액션 대신 턴제 전술**이 코어다.
설계·로드맵: `docs/DESIGN.md`.

## 문서 관리 원칙
- **작업을 마칠 때마다** 이 문서(구조·규칙 변경분)와 `docs/DESIGN.md`(체크리스트·시스템 설명)를 갱신할 것.
- 아래 "다음 후보" 섹션을 항상 최신으로 유지 — 끝낸 항목은 지우고, 새로 떠오른 아이디어를 우선순위와 함께 추가.

## 다음 후보 (우선순위)

> 확장 판단 기준 3가지(절차 생성 우선·재미·검증 가능)와 후보 평가는 `docs/DESIGN.md`의 **"확장 방향"** 참조.
> 각 후보의 착수 방법·건드릴 파일까지 정리한 상세본은 `docs/ROADMAP.md`.

1. **밸런스 조정 ★시급** — 시뮬레이터가 **완주율 0%**를 찾아냈다(`docs/DESIGN.md`의 "밸런스 측정"). 경험치 진행이 적 레벨 상승을 못 따라간다 — 3단계(적 Lv.3)에서 아군은 평균 Lv.1.4. 손볼 후보: ① 경험치를 **파티 전체에 분배**(지금은 처치한 유닛만) ② `expToLevel`을 낮추기 ③ 적 레벨 상승(`1+⌊tier×0.7⌋`)을 완만하게 ④ 4번째 동료를 더 일찍/싸게. **고치고 나서 `__bqsim.campaign()`으로 다시 재는 것까지가 한 작업.**
1.5. **방향키 칸 커서** — 이동·공격을 메뉴에서 빼면서 키보드 전용 조작이 끊겼다. 커서 칸 + Enter로 `onTile`을 부르면 복구된다(작업 작음).
2. **적도 특기를 쓰게** — 지금 스킬은 **아군 전용**이다. `foeAct`가 스킬 후보까지 평가하도록 넓히고, 오크 대장에게 보스다운 특기를 준다. 시뮬레이터로 승률 변화를 바로 볼 수 있다. (단, 1번을 먼저 — 지금 적을 더 세게 만들면 안 된다)
3. **전투 연출 보강** — 이동 경로 따라 미끄러지기, 피격 번쩍·데미지 숫자, 처치 연출, 카메라 살짝 흔들기. (지금은 위치 보간만 있음)
4. **적 AI 고도화** — 지형 활용(숲·언덕 선점), 집중 공격, 사제 우선 노리기, 후퇴 판단.
5. **지형 상호작용** — 높이차 사거리/피해 보정, 물가 밀어내기, 파괴 가능한 바위.
6. **원정 사건 노드** — 갈림길에 이벤트(용병 제안·함정·수수께끼) 추가. 백층 던전의 방 이벤트와 같은 계열.
7. **저장/이어하기** — 원정 중간 상태를 localStorage에 (`bq-run`).
8. **음악** — 절차 생성 BGM(지도/전투/승리). 백층 던전 `music.ts` 패턴 이식.
9. **바둑·오목식 미니게임** — 마을이나 특수 노드에서 즐기는 판 위 미니게임(원작 영감 회수).
10. **모바일 최적화** — 터치 타겟 크기, 세로 화면 카메라 맞춤, DPR 제한.

## 실행
- `npm install` — 최초 1회
- `npm run dev` — 개발 서버 (기본 포트 5176, `PORT` 환경변수로 변경 가능 / 백층 던전 5175와 동시 실행 가능)
- `npm run typecheck` — 타입 검사
- `npm run build` — 프로덕션 빌드

## 검증 (이 프로젝트의 기둥)
- **전투 로직은 순수 함수** (`src/lib/battle.ts`) — 렌더·React 없이 상태를 굴릴 수 있다. 밸런스는 추측이 아니라 **측정**으로 정한다.
- **DEV 훅 `window.__bq`** (프로덕션 제외): `phase()` / `state()`(전투 상태) / `journey()` / `party()` / `gold()` / `tile(x,y)`(3D 클릭과 동일 경로) / `travel(nodeId)` / `hover()`(포인터가 마지막으로 읽힌 칸). 3D 레이캐스트를 흉내 내지 않고도 전 플로우를 자동 조작할 수 있다.
- **DEV 훅 `window.__bqProject(x,y)`** — 칸 중심의 화면 좌표(px). `hover()`와 짝지으면 **클릭이 실제로 그 칸에 맞는지**를 자동으로 검증할 수 있다 (칸 → 화면 → 포인터 이벤트 → 칸 왕복). 192칸 전수 검사가 실측 기준.
  - 주의: 언덕·바위는 옆면이 화면 아래로 뻗으므로, 윗면 중심을 그대로 찍으면 실루엣 경계에 걸린다. 검증 시 **10px쯤 아래**를 찍을 것 (사람이 실제로 누르는 위치).
  - 합성 `PointerEvent`로 검증할 땐 `pointerType`·`isPrimary`·`buttons`를 채울 것 — 빠뜨리면 r3f가 무시해 클릭이 안 먹는 것처럼 보인다.
- **`?rafshim` 쿼리** — 숨김 탭(헤드리스 프리뷰)에서는 크롬이 rAF를 멈춰 3D가 안 그려진다. index.html의 개발용 심이 타이머 rAF + ResizeObserver 폴리필 + **`window.__pump(n)` 동기 프레임 구동** + **뷰포트 0×0이면 1280×800 가상 뷰포트 폴백**을 제공한다.
- 크롬 집중 스로틀링(오래 숨겨진 탭)에서는 `setTimeout`이 죽으므로, 자동 검증은 **MessageChannel 틱**(스로틀 면제) + `__pump`로 구동할 것. 단 **적 턴 자동 진행은 setTimeout 기반**이라 스로틀 상태에서는 느려진다 — 전투 자동 검증은 보이는 탭이 안정적.
- 렌더 확인이 필요하면 `canvas.getContext('webgl').readPixels`로 픽셀 샘플을 떠서 "실제로 뭔가 그려졌는지"를 검사할 수 있다 (스크린샷이 막힌 환경 대비).

## 구조

**기능별 1파일 원칙** — 화면·오버레이는 `src/ui/`에 기능 단위로 분리해 커밋도 기능 단위로 나눈다. 표시(JSX)는 컴포넌트가, 상태 전이·보상·효과음은 App 콜백이 담당한다.
**주의:** 자동화(검증 스크립트·향후 시뮬봇)가 DOM 클래스(`.choice-btn`/`.action-panel`/`.map-node`/`.big-btn`)를 셀렉터로 쓰므로 클래스명은 함부로 바꾸지 말 것.

- `src/App.tsx` — phase 상태 머신 + 배선: `title` → `journey`(길 선택) → 노드별 화면(`battle` / `town` / `treasure` / `camp`) → 다시 `journey` … → 보스 노드 승리 시 `ending`. 전투 중 적 차례는 useEffect가 550ms 간격으로 `foeAct`를 돌린다. 전투 종료 시 체력·레벨·경험치·소지품을 원정 상태로 반영.
- `src/lib/battle.ts` — **턴 엔진 (순수 함수, 이 게임의 심장)**. 라운드마다 속도 순으로 전원 1회 행동, 한 턴 = 이동(선택) + 행동 1회(공격/회복/아이템/대기). 공격 후 상대 사거리 안이면 **반격 60%**. 지형이 공격·방어를 보정. `createBattle` / `doMove`·`undoMove` / `doAttack` / `doSkill`(특기) / `doHeal` / `doItem` / `doWait` / `foeAct`(적 AI) / `previewDamage`(UI 미리보기와 실제 판정이 같은 식 — 스킬 배수도 같은 함수를 탄다).
  - 특기 조회: `canUseSkill`(배웠고·쿨다운 돌았고·맞을 대상이 있는가) / `skillTargets`(고를 수 있는 대상) / `skillVictims`(실제로 맞을 유닛 — 미리보기와 판정이 같은 목록).
- `src/lib/board.ts` — 전투 보드 절차 생성(덩어리 지형: 숲·언덕·물·바위) + **다익스트라 이동 범위**(지형 비용, 적은 통과 불가·아군은 통과 가능하나 정지 불가) + 경로 역추적.
- `src/lib/journey.ts` — 원정 지도 그래프 절차 생성 (단계별 1~3노드, 인접 단계만 연결, 들어오는 길 없는 노드 방지). 중반 **마을 보장**, 보스 직전 **정비 노드 보장**. 노드 종류: 전투/마을/보물/야영/보스.
- `src/lib/encounter.ts` — 단계(tier)에 맞춰 보드·적 편성·배치 생성. **난이도 곡선의 단일 출처** (적 수·레벨·종류 풀·보상 금화).
- `src/lib/units.ts` — 직업 9종(아군 4: 검사·궁수·사제·창병 / 적 5: 고블린·늑대·도적궁수·오크·오크대장) 스탯·성장·경험치. 아군 직업은 `skill`을, 유닛은 `cd`(남은 쿨다운)를 들고 다닌다.
- `src/lib/skills.ts` — **직업 특기(쿨다운제) 순수 데이터.** 검사 🌀회전베기(인접 전체·80%·반격없음) / 궁수 🎯저격(사거리+2·150%·반격없음) / 사제 🙏기도(사거리 내 아군 전체 회복 70%) / 창병 🔱관통(대상+뒤 적). 지금은 **아군 전용** — 적 AI는 아직 스킬을 쓰지 않는다.
  - **쿨다운은 그 유닛의 차례가 돌아올 때 1씩 준다** (라운드 넘어갈 때가 아니라). 라운드 끝에 쓴 느린 유닛이 곧바로 1을 돌려받아 속도에 따라 실제 대기가 달라지던 문제 때문.
- `src/lib/items.ts` — 회복약·폭탄(범위)·숫돌(공격 버프). 전투 중 '행동'으로 사용.
- `src/dev/simBot.ts` — **밸런스 시뮬레이터**(프로덕션 제외). 간이 아군 AI + `foeAct`로 전투를 헤드리스로 굴린다. `runSim`(단계별) / `runSweep`(1~8단계) / `runCampaign`(원정 통째 — 레벨이 실제로 벌려서 오르므로 이쪽이 진짜 난이도). 콘솔 훅 `__bqsim.start/sweep/campaign`.
  - 아군 AI는 `reachableAttacks`·`planAttack`을 그대로 쓴다 — UI가 하이라이트에 쓰는 계산과 **같은 함수**라, 시뮬레이터가 재는 게 실제 플레이와 어긋나지 않는다.
  - node로도 돌릴 수 있다: `npx esbuild <스크립트>.ts --bundle --platform=node --format=esm --define:import.meta.env.DEV=false` 후 `node`.
- `src/lib/rng.ts` — mulberry32 시드 난수 + `nextRand`(상태를 들고 다니는 순수 난수 — 전투 리플레이 가능).
- `src/lib/sound.ts` — Web Audio 합성 효과음(파일 0개). 음소거 `bq-muted`.
- `src/lib/store.ts` — useLocalStorage + `suspendPersistence`(시연·튜토리얼용 저장 격리 스위치). 키 접두사 `bq-`.
- `src/three/BattleScene.tsx` — 보드 렌더(지형 InstancedMesh·하이라이트 InstancedMesh·차례 링) + 말 배치·체력 바 + **클릭 판정**. 화면 비율에 맞춰 보드 전체가 들어오게 카메라를 맞추는 `CameraRig` 포함.
  - **칸 판정은 지형 InstancedMesh를 직접 레이캐스트해 `instanceId`로 얻는다** (`instanceId → [i % w, ⌊i/w⌋]`). 예전엔 보드 위 `y=1.2`에 띄운 투명 평면을 썼는데, 카메라가 약 55° 기울어 있어 **화면상 한 칸 가까이(≈0.86칸) 어긋났다**. 하이라이트·차례 링은 `raycast={NO_RAYCAST}`로 판정에서 빼 클릭을 가리지 않는다.
  - **행동은 포인터를 뗄 때** 일어난다(`beginPress`/`endPress`). 우클릭은 즉시 정보, **450ms 이상 길게 누르면 정보**(행동 안 함), 다른 칸에서 떼면 취소 — 터치에서도 오조작 없이 적을 살펴볼 수 있게.
  - 쓰러진 말은 핸들러를 달지 않아 레이가 통과한다 (그 칸으로 이동하는 클릭을 막지 않도록).
- `src/three/Piece.tsx` — 말(유닛) — 직업별 실루엣을 절차 지오메트리로만 표현.
- `src/ui/Menu.tsx` — **표준 메뉴 선택 UI** (아래 규약 참조).
- `src/ui/BattleHud.tsx` — 차례 순서 바 · 유닛 카드 · **행동 메뉴** · 표적 미리보기(예상 피해·반격) · 전투 기록.
  - **이동·공격은 메뉴에 없다** — 판을 직접 누르는 게 기본. `BattleMode`에는 대상을 골라야 하는 `heal`·`skill`·`item`만 남는다.
  - 특기는 메뉴에 있다(눌러선 뜻이 애매하므로). 대상이 필요 없는 것(회전베기·기도)은 **누르면 즉시 발동**, 대상이 필요한 것(저격·관통)만 `skill` 모드로 들어간다.
- `src/ui/JourneyScreen.tsx` — 원정 지도(SVG 길 + 노드 버튼 + 키보드 선택지).
- `src/ui/TownScreen.tsx` — 마을(회복·물자 구입·동료 영입).
- `src/ui/Screens.tsx` — 타이틀·전투 결과·보물·야영·엔딩.

## 전투 조작 규약 (직접 조작 우선)
**메뉴를 거치지 않는 것이 기본.** 유저가 하려는 게 뻔한 건 판을 눌러 바로 되게 하고, 메뉴에는 *눌러선 뜻이 애매한 것*만 남긴다.
- 🔵 **파란 칸 클릭 = 이동** (모드 진입 없음) · 🔴 **붉은 적 클릭 = 공격** (모드 진입 없음)
- 🟠 **주황 적 클릭 = 이동 후 공격** — 좋은 자리를 `planAttack`이 골라 한 번의 클릭으로 이동+공격까지 한다. "거기까지 가면 닿나"를 유저가 세어 보지 않게 하는 게 목적.
- **우클릭 / 길게 누르기 = 정보만** (행동하지 않음) · 마우스 **호버 = 예상 피해 미리보기**
- 사거리 밖의 적을 누르면 공격 대신 정보 — 헛클릭이 행동으로 새지 않게.
- 대기 중에도 갈 수 있는 칸(파랑)과 지금 때릴 수 있는 적(빨강)을 **항상** 함께 보여 준다. 하이라이트가 곧 설명이라 안내 문구를 줄일 수 있다.
- 메뉴에 남는 것: 회복(사제) · 아이템 · 이동 취소 · 대기.
- ⚠️ 이동·공격을 메뉴에서 뺀 대가로 **키보드만으로는 이동·공격을 할 수 없다.** 되살리려면 방향키 칸 커서가 필요 (로드맵 참조).

## 표준 메뉴 선택 UI (`src/ui/Menu.tsx`)
백층 던전에서 검증된 규약을 그대로 이식했다. 모든 선택 화면에서:
- **항상 한 항목이 하이라이트**(`.focused` 금색 테두리, 기본 = 첫 활성 항목)
- **방향키로 이동**(순환·비활성 스킵), **Enter/Space로 확정**, 숫자 1~9는 바로 선택
- 마우스 호버는 하이라이트와 동기화, 클릭은 즉시 확정
- 메뉴가 겹치면 **나중에 열린 메뉴만 키를 받음**(모듈 레벨 메뉴 스택)
- 자동 반복(`e.repeat`) 무시

## 원칙
- UI 문구는 **한국어**. 백엔드 없음 — localStorage만.
- **외부 에셋 금지** — 절차 생성 지오메트리 + 이모지/캔버스 + Web Audio 합성으로 해결. 부득이 추가 시 출처·라이선스를 문서에 기록. (폰트 Jua만 예외 — SIL OFL)
- **게임 로직은 렌더와 분리된 순수 함수 지향**. 전투·지도·편성은 React 없이 돌 수 있어야 한다 (시뮬레이터 전제).
- **시드 고정** — 같은 시드는 같은 지도·같은 보드. 재현 가능해야 디버깅도 밸런싱도 된다.
- 커밋 기록이 곧 작업 증빙 — 작업 단위마다 의미 있는 **한국어 커밋 메시지**로 커밋하고, 작업이 끝나면 바로 push.

## 관련
- 자매작(원작): **백층 던전** — https://github.com/Hakhyun-Kim/dungeon100 (실시간 액션 로그라이크). 절차 생성·검증 패턴·표준 메뉴 UI·rafshim 검증 심을 여기로 이식했다.
